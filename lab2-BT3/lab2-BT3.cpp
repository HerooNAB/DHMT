// lab2-BT3.cpp : Defines the entry point for the console application.
//
...
float navigationRotation[3] = { 10.0, 180.0, 0.0 };
char pixelstring[30];
int cframe = 0;
int time = 0;
int timebase = 0;
...
void mouseMotionFunc(int x, int y)
{
	...
	if (leftMouseButtonActive)
	{
		...
		navigationRotation[1] += ((mousePressedX - y) * 180.0f) / 200.0f;
		mousePressedY = y;
		mousePressedX = x;
	}
	//panning
...
{
	printf("simpleGLUT\n\tGordon Werzstein [ gordon.werzstein@medien.ini-weimar.de]\n\n");
	printf("keys:\n\tf\t- toggle fulscreen\n\tesc\t-exit\n\n");
